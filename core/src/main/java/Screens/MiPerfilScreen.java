...
public class MiPerfilScreen extends BaseScreen {
    ...
    protected void onShow() {
        ...
        if (u != null && u.historial != null) {
            partidasJugadas = u.historial.size();
        }
        ...
        for (int n = 1; n <= 7; n++) {
            ...
            if (u != null) {
                try {
                    promedio = u.getTiempoPromedioNivel(n);
                } catch (Throwable ignored) {
                }
                try {
                    mejorTiempo = u.getMejorTiempoPorNivel(n);
                } catch (Throwable ignored) {
                }
                try {
                    bestSteps = u.getMayorPuntuacion(n);
                } catch (Throwable ignored) {
                }
                try {
                    partidasNivel = u.getPartidasPorNivel(n);
                } catch (Throwable ignored) {
                }
            }
        ...
        btnStyleCached.up = btnClearDr;
        btnStyleCached.down = btnClearDr;
        btnStyleCached.over = btnClearDr;
        btnStyleCached.checked = btnClearDr;
        btnStyleCached.disabled = btnClearDr;
    }
    ...
    private void reloadAvatar() {
        ...
        if (avatarTex == null) {
            avatarTex = new Texture(files.internal(path));
        }
        if (avatarImg != null) {
    ...
    private void onEditUsuario() {
        ...
        Table c = dlg.getContentTable();
        c.pad(16f);
        c.defaults().pad(6f).fillX();
    ...
    private void onEditPass() {
        ...
        tf1.setPasswordMode(true);
        tf1.setPasswordCharacter('*');
        tf2.setPasswordMode(true);
        tf2.setPasswordCharacter('*');
        final Label err = new Label("", new Label.LabelStyle(smallFont, Color.SALMON));
        ...
        ok.addListener(new ClickListener() {
            ...
            public void clicked(InputEvent e, float x, float y) {
                ...
                if (!isAlnum(p1) || !isAlnum(p2)) {
                    err.setText("Solo letras o numeros.");
                    return;
                }
                if (!p1.equals(p2)) {
                    err.setText("No coincide.");
                    return;
                }
                Usuario u = ManejoUsuarios.UsuarioActivo;
                if (u != null) {
    ...
    private void forceWhiteText(Actor a) {
        if (a instanceof Label) {
            ((Label) a).setColor(Color.WHITE);
        }
        if (a instanceof TextButton) {
            ((TextButton) a).getLabel().setColor(Color.WHITE);
        }
        if (a instanceof Group) {
            for (Actor ch : ((Group) a).getChildren()) {
                forceWhiteText(ch);
            }
        }
    }
